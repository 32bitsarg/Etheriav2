...
export async function getMe(userId: string) {
  ...
  if (!user) return null;
  const city = await prisma.city.findFirst({ where: { userId }, select: { id: true, worldId: true } });
  return { user: { id: user.id, email: user.email, name: user.name ?? "Commander" }, cityId: city?.id ?? null, worldId: city?.worldId ?? null };
}
